...
| REQ-MDA-001〜002、REQ-AST-001〜002 | `scan-pipeline.ja.md`、`architecture.ja.md` metadata不足/asset・画像候補、`ui.ja.md` Metadata不足/Image destination conflict | T15、T16、T28、T31、T34 | Core strategy unit test、`metadata_fixtures.rs`、metadata不足設定workflow integration test、画像候補workflow integration test、UI typecheck/build |
...
- T34: metadata不足の既定skipと明示opt-in、`Unknown Artist` / `Unknown Album` 補完、rules snapshot、CLI/Desktop設定、および後方互換。
