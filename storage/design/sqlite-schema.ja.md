...
| scan_items | scan_run_id, file_id, disposition, warning | scan の snapshot |
| plan_runs | id, scan_run_id, rules_version, snapshot_hash, status | immutable plan 親 |
| plan_items | id, plan_run_id, file_id, ordinal, action, source_path, target_path, conflict, risk, reason | apply 入力 |
...
初期実装では plan 作成時に `scan_runs.status = completed` を必須とし、`plan_runs` と `plan_items` を同じDBに保存する。plan itemは targetを再計算せず、後続applyの唯一の入力になる。
