...
- [x] T27: Coreのpath長診断へ対象・実測文字数・上限文字数を追加する。上限以下を許可する境界test、Plan/命名preview/CLI/Desktopに現れる全理由codeの日本語変換と未知codeの日本語fallback、内部code非表示、および既存 `path_too_long` riskとの後方互換を追加する（T02、T13、T18依存）。
