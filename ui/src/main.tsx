...
type PlanConflictDetail = { id: string; kind: string; target_path: string; existing_target_path?: string; members: { item_id: string; ordinal: number; source_path: string }[] };
...
function App() {
  ...
  return <main>
    ...
    <section><h2>{ja.logs}</h2><input placeholder={ja.logSearch} value={logQuery} onChange={event => setLogQuery(event.target.value)} /><select value={logResult} onChange={event => setLogResult(event.target.value)}><option value="">{ja.all}</option><option value="success">{ja.success}</option><option value="failed">{ja.failed}</option><option value="skipped">{ja.skipped}</option></select>
      <div className="virtual" ref={logRef}><div style={{ height: logVirtualizer.getTotalSize(), position: "relative" }}>{logVirtualizer.getVirtualItems().map(row => { const value = logs[row.index]; return <div className="row" data-index={row.index} ref={logVirtualizer.measureElement} key={value.id} style={{ position: "absolute", top: 0, width: "100%", transform: `translateY(${row.start}px)` }}><b>#{value.sequence_no}</b><span>{value.result}</span><code>{value.action} {value.source_path} → {value.target_path}</code>{value.error && <small>{formatReason(value.error)}</small>}</div>; })}</div></div><button disabled={!executionId} onClick={() => void loadLogs()}>{ja.more}</button>{metrics.map(value => <output key={`${value.phase}-${value.elapsed_ms}`}>{value.phase}: {value.elapsed_ms}ms / {value.item_count}件</output>)}
    </section>
    <section><div className="section-title"><h2>{ja.planned}</h2>{plan && <strong>{planTotal.toLocaleString()}件</strong>}</div>
      ...
      <div className="virtual plan-list" ref={planRef}><div style={{ height: planVirtualizer.getTotalSize(), position: "relative" }}>{planVirtualizer.getVirtualItems().map(row => { const value = items[row.index]; const groupId = value.conflict_group_id; const detail = groupId ? conflictDetails[groupId] : undefined; return <article className="plan-item" data-index={row.index} ref={planVirtualizer.measureElement} key={value.id} style={{ position: "absolute", top: 0, width: "100%", transform: `translateY(${row.start}px)` }}><header><b>#{value.ordinal}</b><span className="badge">{actionLabels[value.action] ?? value.action}</span>{value.risk !== "none" && <span className="badge risk">{riskLabels[value.risk] ?? value.risk}</span>}</header><div className="path-line"><span>元</span><code title={value.source_path}>{value.source_path}</code></div><div className="path-line"><span>移動先</span><code title={value.target_path}>{value.target_path ?? "—"}</code></div>{value.reason && <div className="reason"><span>理由</span><span>{formatReason(value.reason)}</span></div>}{groupId && groupId in conflictDetails && <div className="conflict-detail">{detail ? <><div className="path-line"><span>共通先</span><code title={detail.target_path}>{detail.target_path}</code></div><strong>この移動先になるファイル（{detail.members.length}件）</strong>{detail.members.map(member => <div className="conflict-member" key={member.item_id}><span>#{member.ordinal}</span><code title={member.source_path}>{member.source_path}</code><button className="secondary compact" onClick={() => void navigator.clipboard.writeText(member.source_path)}>コピー</button></div>)}</> : <span>衝突相手を読み込んでいます…</span>}</div>}<footer>{groupId && <button className="secondary" onClick={() => void toggleConflict(value)}>{groupId in conflictDetails ? "衝突相手を閉じる" : `衝突相手を表示（${value.conflict_member_count}件）`}</button>}<button className="secondary" disabled={!plan || busy} onClick={() => { const target = prompt("新しい移動先（改訂Planを作成します）", value.target_path ?? ""); if (target && plan) void run<Workflow>(() => invoke("revise_plan_target", { database, planId: plan.id, planItemId: value.id, target }), setPlan); }}>移動先を変更</button></footer></article>; })}</div></div>
