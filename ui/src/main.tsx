...
const ja = {
  ...
  scan: "Scan", plan: "Plan", dry: "Dry-run", apply: "Apply", verify: "Verify", rollback: "Rollback dry-run", cancel: "Scanを取消",
...
function App() {
  ...
  return <main>
    ...
    <section><h2>{ja.workflow}</h2><label>{ja.source}<input value={source} onChange={event => setSource(event.target.value)} /></label><label>{ja.target}<input value={target} onChange={event => setTarget(event.target.value)} /></label><label>{ja.database}<input value={database} onChange={event => setDatabase(event.target.value)} /></label>
      <div className="actions"><button disabled={!source || busy} onClick={() => void startScan()}>{ja.scan}</button><button disabled={!scanRequest || scanRequest.status !== "running"} onClick={() => void invoke("cancel_scan", { scanId: scanRequest?.request_id })}>{ja.cancel}</button><button disabled={!scan || !target || busy} onClick={() => run<Workflow>(() => invoke("create_plan", { scanId: scan?.scan_id, target, database }), setPlan)}>{ja.plan}</button><button disabled={!plan || busy} onClick={() => run<Workflow>(() => invoke("apply_plan", { planId: plan?.id, database, execute: false }), value => { setExecution(value); setExecutionId(value.id); })}>{ja.dry}</button><button className="danger" disabled={!plan || busy} onClick={() => plan && confirm(`保存済みPlan ${plan.id} のsnapshotを検証して本実行します。`) && run<Workflow>(() => invoke("apply_plan", { planId: plan.id, database, execute: true }), value => { setExecution(value); setExecutionId(value.id); })}>{ja.apply}</button><button disabled={!executionId || busy} onClick={() => run<Workflow>(() => invoke("verify_execution", { executionId, database }), setExecution)}>{ja.verify}</button><button disabled={!executionId || busy} onClick={() => run<Workflow>(() => invoke("rollback_execution", { executionId, database, execute: false }), setExecution)}>{ja.rollback}</button></div>
