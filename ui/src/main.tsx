...
function App() {
  ...
  return <main>
    ...
    <section><h2>{ja.history}</h2><button onClick={() => void loadHistory(true)}>{ja.refresh}</button><div className="history">{history.map(value => <button className="row" key={value.id} onClick={() => void restoreHistory(value)}><b>{value.kind}</b><span>{value.status}</span><code>{value.id}</code></button>)}</div><button onClick={() => void loadHistory()}>{ja.more}</button></section>
  </main>;
}
